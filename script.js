...
planets.forEach(planets => {
    console.log(`Name: ${planets.name}, Distance from Sun (million km): ${planets.distance}, Diameter (km): ${planets.diameter}`);
});

//Function to calculate average Diameter of planets
function averageDiameter(planets) {
    const totalDiameter = planets.reduce((sum, planet) => sum + planet.diameter, 0);
...
function displayPlanet() {
    ...
    planetIndex.textContent = `Planet ${currentPlanetIndex + 1} of ${planets.length}`;
